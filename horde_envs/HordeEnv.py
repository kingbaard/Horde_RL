...
class HordeEnv(gym.Env):
    def __init__(self, render_mode) -> None:
        self.render_mode = render_mode

        self.observation_space = spaces.Dict(
            {
                "agent": spaces.Box(0, WIDTH - 1, shape=(2,), dtype=int),
                "enemy1_zone": spaces.Discrete(8),
                "enemy1_dist": spaces.Discrete(3),
                "enemy2_zone": spaces.Discrete(8),
                "enemy2_dist": spaces.Discrete(3),
                "enemy3_zone": spaces.Discrete(8),
                "enemy3_dist": spaces.Discrete(3),
                "enemy4_zone": spaces.Discrete(8),
                "enemy4_dist": spaces.Discrete(3),
                "enemy5_zone": spaces.Discrete(8),
                "enemy5_dist": spaces.Discrete(3),
            }
        )

        self.action_space = spaces.Dict(
            {
                "hor_dir": spaces.Discrete(3),
                "her_dir": spaces.Discrete(3),
                "shoot_dir": spaces.Discrete(5),
            }
        )
        self.reset()
    ...
    def get_state(self):
        ...
        player_pos = np.array([int(self.plyaer.x), int(self.player.y)])
        ...
        for e in self.enemies:
            ...
            state.append(enemy_theta_zone)
            state.append(e_distance_category)
    ...
    def get_screenshot(self):
        surface_data = pygame.image.tostring(self.screen, 'RGB')
        cv_image = np.fromstring(surface_data, dtype=np.uint8).reshape(HEIGHT, WIDTH, 3)
        cv_image = cv2.cvtColor(cv_image, cv2.COLOR_RGB2GRAY)
        cv_image = cv2.resize(cv_image, None, fx=0.125, fy=0.125, interpolation=cv2.INTER_AREA)

        intensity_levels = {0:0, 76:1, 150:2, 255:3}
        simple_image = np.zeros_like(cv_image)
        for i in range(len(cv_image)):
            for j in range(len(cv_image[i])):
                if cv_image[i][j] < 38:
                    simple_image[i][j] = intensity_levels[0]
                elif cv_image[i][j] < 113:
                    simple_image[i][j]  = intensity_levels[76]
                elif cv_image[i][j] < 203:
                    simple_image[i][j]  = intensity_levels[150]
                else:
                    simple_image[i][j] = intensity_levels[255]
