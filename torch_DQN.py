...
def plot_rewards():
    ...
    plt.clf
    plt.plot([i for i in range(1, len(episode_rewards)+1)], episode_rewards)
...
def decode_state(index):
    ...
    for i in range(5, 1, -1):
        ...
        e_zone = index // 8
        index = index % 8
        state.update({f"enemy{i}_zone":e_zone})
        state.update({f"enemy{i}_dist":e_distance})
    state.update("")
